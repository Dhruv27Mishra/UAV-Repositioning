...
def train(num_episodes=1000,
    ...
    np.save("results/qmix_fairness.npy", np.array(episode_fairnesses))  # change to vdn_fairness.npy in train_vdn.py
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("results/qmix_fairness_curve.png")  
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/qmix_fairness_vs_density.png")
    plt.show()
    np.save(f"results/qmix_fairness_vs_density.npy", np.array(fairness_scores))
